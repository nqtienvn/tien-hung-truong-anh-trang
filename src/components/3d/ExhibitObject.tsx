...
const PaintingComponent: React.FC<{
  ...
  isVisible?: boolean;
  onClick?: (exhibit: Exhibit) => void;
}> = ({ exhibit, isSelected, hovered, setHovered, setSelectedExhibit, setExhibitModalMode, language, isVisible = true, onClick }) => {
  ...
  useEffect(() => {
    ...
    // Luôn dựng trước một texture nội bộ để tranh không bị trống khi ảnh mạng chậm/lỗi.
    ...
    loader.load(
      ...
      undefined,
      (err) => {
        console.warn('Lỗi tải texture tranh, giữ ảnh vẽ thủ công:', exhibit.thumbnail_url, err);
        setTextureLoading(false);
      }
    );
  }, [exhibit.thumbnail_url, language]);

  return (
    <group>
      {/* Group meshes: luôn hiển thị để người chơi thấy tranh trên tường */}
      <group 
      ...
      >
        ...
        >
          ...
          {texture && !textureError ? (
            ...
            // Placeholder màu vàng kem khi đang load
            <meshStandardMaterial 
              color="#c9a96e" 
              roughness={0.9}
            />
          ) : (
            // Lỗi: hiển màu nâu đỏ bất khả kháng
            <meshStandardMaterial 
              color="#8a4a2f" 
              roughness={0.8}
            />
        ...
        {/* 3. Bảng thông tin dạng stand cao cấp đặt ngoài dây chắn - bấm để mở popup */}
        <group
        ...
        >
          {/* Vùng bắt click riêng của bệ: ngăn raycast xuyên qua bệ trúng tranh phía sau */}
          ...
          {/* Mặt bảng tối màu có viền vàng đồng */}
          ...
          {/* Điểm nhấp nháy báo hiệu có thể bấm để đọc nội dung */}
...
const SculptureComponent: React.FC<{
  ...
  isVisible?: boolean;
  onClick?: (exhibit: Exhibit) => void;
}> = ({ exhibit, isSelected, hovered, setHovered, setSelectedExhibit, language, meshRef, isVisible = true, onClick }) => {
  return (
    <group>
      ...
      >
        ...
        >
          <div className="w-[180px] bg-slate-900/95 text-white p-2.5 rounded-lg border border-slate-700 font-sans shadow-2xl pointer-events-none select-none">
            <p className="text-[12px] font-bold text-amber-400 leading-tight">
              {language === 'vi' ? exhibit.title.vi : exhibit.title.en}
            </p>
            <p className="text-[9px] text-slate-400 mt-1 leading-none font-sans">
              {language === 'vi' ? exhibit.author.vi : exhibit.author.en}
            </p>
            {isVisible && (
              <p className="text-[7px] text-white/40 mt-1.5 uppercase tracking-widest border-t border-slate-800 pt-1 font-sans">
                Nhấp để khám phá
              </p>
            )}
          </div>
