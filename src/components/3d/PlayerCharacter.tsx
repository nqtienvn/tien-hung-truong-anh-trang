...
export const PlayerCharacter: React.FC = () => {
  ...
  const checkCollision = (x: number, z: number): boolean => {
    ...
    // ── COLLISIONS FOR ROOM 1 (gallery-paintings) ──
    if (galleryId === 'gallery-paintings') {
      // 1. Tường ngăn tại Z = 3.0: Cổng mở ở bên trái X từ -5.0 đến -2.0
      if (z > 2.7 && z < 3.3) {
        if (x < -5.0 || x > -2.0) {
          return true;
        }
      }
      // 3. Vách ngăn phụ phòng tranh tại Z = 13.0
    ...
    // ── COLLISIONS FOR ROOM 2 (gallery-sculptures) ──
    if (galleryId === 'gallery-sculptures') {
      // 2. Tường ngăn tại Z = -3.0: Cổng mở ở bên phải X từ 2.0 đến 5.0
      if (z > -3.3 && z < -2.7) {
        if (x < 2.0 || x > 5.0) {
          return true;
        }
